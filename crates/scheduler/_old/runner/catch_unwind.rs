...
impl<'a> Future for CatchUnwindTaskFuture<'a> {
    type Output = eden_utils::Result<TaskResult, RunTaskError>;
    ...
    fn poll(
        ...
        let result = match catch_unwind(move || future.poll_unpin(cx)) {
            ...
            Ok(Poll::Ready(value)) => value.transform_context(RunTaskError),
            Err(error) => Err(error),
        };
...
#[track_caller]
fn catch_unwind<F: FnOnce() -> R, R>(f: F) -> eden_utils::Result<R, RunTaskError> {
    #[derive(Debug)]
