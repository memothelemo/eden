...
use self::inner::WorkerInner;
use self::task_manager::{PerformTaskAction, WorkerTaskManager};
...
#[derive(Clone)]
pub struct Worker<S>(Arc<WorkerInner<S>>);
...
impl<S: Clone + Send + Sync + 'static> Worker<S> {
    ...
    pub fn new(id: WorkerId, pool: sqlx::PgPool, settings: &Settings, state: S) -> Self {
        Self(Arc::new(WorkerInner {
            ...
            state,
            task_manager: WorkerTaskManager::new(settings.max_running_tasks.get(), id),
...
impl<S: Clone + Send + Sync + 'static> Worker<S> {
    ...
    pub async fn start(&self) -> Result<(), WorkerStartError> {
        ...
        *handle = Some(eden_utils::tokio::spawn(
            ...
            self::runner::WorkerRunner::new(worker_tx).run(),
        ));
