...
use super::task_manager::WorkerTaskManager;
use super::Worker;
...
pub struct WorkerInner<S> {
    ...
    pub state: S,
    pub task_manager: WorkerTaskManager,
...
impl<S: Clone + Send + Sync + 'static> Debug for WorkerInner<S> {
...
impl<S: Clone + Send + Sync + 'static> Debug for Worker<S> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        self.0.fmt(f)
    }
}
